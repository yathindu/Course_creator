...
from professional.courseware_extraction import (
    answer_question,
...
with col_source:
    with st.expander("1. Source material", expanded=not st.session_state["source_text"]):
        ...
        # Constrained to exactly the 6 required training areas (3 roles x 2 skill
        # areas) rather than free text, so course id/subject stays consistent across
        # runs instead of drifting ("Marketing" vs "marketing" vs "Marketing Dept").
        subject = col1.selectbox("Role", ["Marketing", "Cashiers", "Back Officers"])
        skill_area_choice = col2.selectbox("Skill area", ["Technical Skills", "Soft Skills"])
        skill_area = "soft_skills" if skill_area_choice == "Soft Skills" else "technical"
        ...
        if st.button("Extract content", disabled=not uploads):
            ...
            try:
                ...
                st.session_state["course_meta"] = {"skill_area": skill_area, "subject": subject, "medium": medium}
    ...
    with st.expander("2. Chapters / modules", expanded=not st.session_state.get("editing_lesson_id")):
        ...
        if st.button("Generate groupings"):
            with st.spinner("Analyzing structure..."):
                try:
                    groupings = generate_groupings(st.session_state["source_text"], meta["skill_area"], meta["subject"], meta["medium"])
...
# ---------------------------------------------------------------------------
ACTIVITY_TYPES = ["content", "vocabulary", "mcq", "image_selection", "true_false", "fill_blank", "ordering", "listening", "speaking", "video"]
...
def _activity_editor(activity, key_prefix):
    ...
    if t == "content":
        activity["body"] = st.text_area(
            "Content (teaching text the learner reads and hears narrated)",
            activity.get("body", ""), key=f"{key_prefix}_body", height=150)
            ...
            with st.spinner("Drafting a suggestion..."):
                try:
                    drafted = generate_single_activity(
                        ...
                        lesson["skill_area"], lesson["subject"], lesson["medium"],
                    )
