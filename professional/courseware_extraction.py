...
from professional.courseware_schema import (
    Activity,
    GeneratedLesson,
    Grouping,
    GroupingsResult,
...
def skill_area_label(skill_area):
    """Natural-language phrasing for a skill_area value, for splicing into prompts."""
    return "soft skills" if skill_area == "soft_skills" else "technical skills"
...
# ---------------------------------------------------------------------------
GROUPINGS_SYSTEM = """You are organizing a training manual's content into teaching chapters/modules
for a {subject} staff training course focused on {skill_area_label} (medium: {medium}).

Read the source content and split it into logical groupings (chapters/modules) a trainer would
...
def generate_groupings(source_text, skill_area, subject, medium):
    chain = _groupings_prompt | _llm().with_structured_output(GroupingsResult)
    result = _invoke_with_retry(lambda: chain.invoke({
        "source_text": source_text, "skill_area_label": skill_area_label(skill_area), "subject": subject,
...
# ---------------------------------------------------------------------------
LESSON_SKELETONS_SYSTEM = """You are planning lessons for one chapter/module of a {subject} staff
training course focused on {skill_area_label} (medium: {medium}).
...
def generate_lesson_skeletons(grouping: Grouping, skill_area, subject, medium):
    ...
    result = _invoke_with_retry(lambda: chain.invoke({
        ...
        "skill_area_label": skill_area_label(skill_area),
        "subject": subject,
        "medium": medium,
...
ACTIVITY_TYPE_HINTS = {
    "content": "body (REQUIRED -- a paragraph of instructional/explanatory text the learner reads "
               "and hears narrated -- teaching content, not a question)",
...
ACTIVITIES_SYSTEM = """You are writing the activities for one lesson of a {subject} staff training
course focused on {skill_area_label} (medium: {medium}). Write all learner-facing text (title,
instructions, body, question, word, definition, sentence, options, prompt_text) in {medium_language}.

This is workplace training for adult staff, not an academic course -- rewrite the source material's
phrasing into concise, practical, on-the-job language. Explain what to actually do/know at work,
not textbook theory. Avoid academic register (no "this unit will examine...", no citations/authors).
...
repetitive. Structure the lesson as teaching followed by checks: open with 1-3 "content" activities
that actually teach the material (each a self-contained paragraph a learner reads/hears), then
follow with quiz-type activities that check understanding of what those content activities just
taught -- don't quiz on something never explained. Available types and their required fields:
""" + _ACTIVITY_TYPE_HINTS_BLOCK + """
...
def generate_lesson_activities(lesson: LessonSkeleton, source_text, skill_area, subject, medium):
    ...
    result.skill_area = skill_area
    result.subject = subject
    result.medium = medium
    return prune_lesson_fields(result)


SINGLE_ACTIVITY_SYSTEM = """You are writing ONE new activity to add to an existing lesson of a
{subject} staff training course focused on {skill_area_label} (medium: {medium}). Write all
learner-facing text (title, instructions, body, question, word, definition, sentence, options,
prompt_text) in {medium_language}. This is workplace training for adult staff -- use concise,
practical, on-the-job language, not academic phrasing.
...
def generate_single_activity(activity_type, activity_id, lesson_title, lesson_description, source_text, skill_area, subject, medium):
...
# ---------------------------------------------------------------------------
QA_SYSTEM = """You are answering a trainer's questions about a source document they've uploaded, for
a {subject} staff training course focused on {skill_area_label} (medium: {medium}). Answer in
{medium_language}, matching the source's language.
...
def answer_question(question, source_text, history, skill_area, subject, medium):
    ...
    result = _invoke_with_retry(lambda: chain.invoke({
        ...
        "skill_area_label": skill_area_label(skill_area), "subject": subject, "medium": medium,
        "medium_language": medium_language(medium),
    }))
    return result.content.strip()
...
def assign_media_filenames(lesson: GeneratedLesson, language):
    for activity in lesson.activities:
        ...
        if activity.type == "content":
            activity.audio = f"audios/{activity.id}_content.mp3"
