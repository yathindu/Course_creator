...
Which activity types need media, and what gets generated:
    content         -> audio from `body` (narrated), no image
...
Image style is "illustration" or "photographic". An activity/option can set its own
"style" field in the JSON to override the run's default (--style, itself
defaulting to "illustration" -- professional/flat, suited to workplace training).
...
freeform-prompt flow uses an LLM to decide which tool to call.

personalization.py's LangChain system+user prompt chain always rewrites
*what* gets generated for images, video, and content/vocabulary narration
audio, using the lesson's own skill_area/subject/medium as context.
...
Usage:
    python generate_lesson_media.py lessons-your_lesson_id.json
    python generate_lesson_media.py lessons-your_lesson_id.json --out content
    python generate_lesson_media.py lessons-your_lesson_id.json --dry-run
    python generate_lesson_media.py lessons-your_lesson_id.json --style photographic --skip-existing=false
"""
...
from professional.personalization import personalize
...
def collect_media_tasks(lesson):
    ...
    for activity in lesson["activities"]:
        ...
        if activity_type == "content":
            if activity.get("audio") and activity.get("body"):
                audio_tasks.append({
                    "activity_id": activity_id,
                    "activity_type": activity_type,
                    "text": activity["body"],
                    "personalizable": True,  # narration of teaching content, not an answer -- safe to rewrite
...
# Orchestration
# ---------------------------------------------------------------------------
def run(lesson_path, out_dir, default_style, skip_existing, dry_run):
    ...
    print(f"Lesson: {lesson['id']} ({lesson.get('subject')}, {lesson.get('skill_area')}, "
          f"language={language})")
    ...
    for t in image_tasks:
        ...
        if skip_existing and target.exists():
            ...
            continue
        style = t["style"] or default_style
        enriched = personalize(t["subject"], t["activity_type"], "image", style, learning_context)
    ...
    for t in audio_tasks:
        ...
        text = t["text"]
        if t["personalizable"]:
            text = personalize(t["text"], t["activity_type"], "audio", "narration", learning_context)
        try:
    ...
    for t in video_tasks:
        ...
        if skip_existing and target.exists():
            ...
            continue
        scene_description = personalize(t["scene_description"], "video_scene", "video", "cinematic", learning_context)
        try:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--style", choices=["illustration", "photographic"], default="illustration",
                         help="Default image style; an activity/option can override this via its own \"style\" field in the JSON (default: illustration)")
    ...
    run(args.lesson_json, args.out, args.style, args.skip_existing, args.dry_run)
