"""
UI for generating and approving lesson media, for adult workplace training.

- HF ("LLM1") is tried first; a second button lets the user escalate the
  same content to OpenRouter ("LLM2") for comparison, showing both results
  and real costs side by side.
- The user only ever sees editable content text, never the underlying
...
- Every approval also gets backed up to GitHub (see repo_persistence.py) if
  GITHUB_TOKEN is configured -- Streamlit Cloud's container disk is
  ephemeral, so without this a crash/restart could lose approved work that
  was never downloaded locally.
- personalize() receives the lesson's skill_area/subject/medium as learning
  context, so generated wording matches the actual course/skill area.
...
try:
    for _key in ("HF_TOKEN", "OPENROUTER_API_KEY", "GITHUB_TOKEN"):
...
import professional.repo_persistence as repo_persistence
from professional.image_processing import BACKGROUND_COLORS, flatten_on_background, has_transparency
from professional.personalization import is_sinhala, personalize, translate_to_english

# Data (drafts/, generated_content/, teacher_profile.json, etc.) lives next to this
# file inside the professional/ package, not in the process's cwd -- the unified app
# is launched from unified_courses/ (so both student/ and professional/ packages can
# coexist), which would otherwise resolve these bare relative paths to the wrong
# (shared, ambiguous) location.
_BASE_DIR = Path(__file__).resolve().parent


def _backup_to_github(local_path, message):
    """Best-effort backup of an approved file to GitHub (see repo_persistence.py) --
    silently does nothing if GITHUB_TOKEN isn't configured. A failure here is
    reported but never treated as the approval itself failing: the local file and
    approvals.json entry are already written by the time this runs, so the user's
    approval is safe regardless of whether the backup succeeds."""
    if not repo_persistence.enabled():
        return
    repo_path = Path(local_path).resolve().relative_to(_BASE_DIR).as_posix()
    try:
        repo_persistence.push_file(local_path, repo_path, message)
        st.caption(f"☁️ Backed up to GitHub ({repo_path})")
    except Exception as e:
        st.caption(f"⚠️ GitHub backup failed (local copy is still safe): {e}")
...
with st.container(border=True):
    ...
    if "id" not in lesson or "activities" not in lesson:
        ...
        st.stop()
    st.caption(f"Loaded **{lesson['id']}** -- {lesson.get('subject', '?')} "
               f"({lesson.get('skill_area', '?')}), {lesson.get('medium', '?')} medium.")

language = hf.resolve_language(lesson.get("medium"))
learning_context = {"skill_area": lesson.get("skill_area"), "subject": lesson.get("subject"), "medium": lesson.get("medium")}
...
with st.container(border=True):
    ...
    def _question_label(aid):
        ...
        # Lesson-authored "group" (e.g. several activities under one theme) is purely
        # a visual clustering hint here -- prefixing the label keeps grouped questions
        # visibly adjacent/labeled in the flat picker without restructuring it into
        # real nested selection.
...
style = task.get("style") or "illustration"
remove_bg = False
if kind == "image":
    with st.container(border=True):
        st.subheader("🎨 Image options")
        icol1, icol2 = st.columns(2)
        style = icol1.selectbox("Style", ["illustration", "photographic"], index=0 if style == "illustration" else 1,
                ...
                def _compose_batch_content():
                    for t in sibling_tasks:
                        st.session_state[f"batch_content_{t['out_path']}"] = personalize(
                            t["subject"], t["activity_type"], "image", style, learning_context)
                ...
                if st.session_state.get("batch_key") == task["activity_id"] and "batch_results" in st.session_state:
                    ...
                    if st.button(f"Approve & Save all {len(batch_results)} options", type="primary"):
                        ...
                        for out_path, info in batch_results.items():
                            ...
                            _backup_to_github(final_path, f"Approve image for {task['activity_id']}: {out_path}")
                        approvals_path.write_text(json.dumps(approvals, indent=2, ensure_ascii=False),
                                                   encoding="utf-8")
                        _backup_to_github(approvals_path, f"Update approvals for {lesson['id']}")
                        ...
                        # on the student/ package's identical code (this file's literal-copy source).
                        st.rerun()
...
# Include style in the key for images so switching illustration<->photographic re-composes the
# content automatically -- otherwise a leftover illustration-flavored description could stay
# paired with a photographic-style generation (or vice versa), the same style/content mismatch
# bug the original cartoon/natural styles had.
...
if st.session_state.get("content_selection_key") != content_selection_key:
    ...
    if _saved_draft is not None:
        ...
        with st.spinner("Composing a suggested prompt..."):
            st.session_state["content_text"] = personalize(
                raw_content, task["activity_type"], kind, _behaviour(), learning_context)
    else:
...
with st.container(border=True):
    ...
    if can_personalize:
        st.caption("Auto-composed for this activity's type and the lesson's context. "
                   "Edit freely, or regenerate.")
...
if "hf_result" in st.session_state:
    with st.container(border=True):
        ...
        if st.button("Approve & Save", type="primary"):
            ...
            _backup_to_github(final_path, f"Approve {kind} for {task['activity_id']}: {task['out_path']}")
            _backup_to_github(approvals_path, f"Update approvals for {lesson['id']}")
            ...
            # (via the student/ package's identical code, this file's literal-copy source).
            st.rerun()
                ...
                if _saved_motion_draft is not None:
                    ...
                    with st.spinner("Composing a suggested motion description..."):
                        st.session_state["motion_text"] = personalize(
                            source_content, task["activity_type"], "animation", "animation", learning_context)
            ...
            if "anim_result" in st.session_state:
                ...
                if _show_media("video", anim_path, state_keys=("anim_result",)):
                    ...
                    if st.button("Approve & Save animation"):
                        ...
                        _backup_to_github(final_anim_path, f"Approve animation for {task['activity_id']}: {anim_out_name}")
                        _backup_to_github(approvals_path, f"Update approvals for {lesson['id']}")
