...
Image / Veo 3.1 Lite): pick a lesson JSON + activity, set
Style (illustration/photographic -- image only), Generate, Play/Preview. Superseded by
...
from professional.generate_lesson_media import collect_media_tasks, generate_audio, resolve_language
from professional.openrouter_pipeline import generate_image, generate_video
...
lesson_path = st.text_input("Lesson JSON path", "")
...
# --- 1: system prompt (style/mood/illustration-or-photographic), image tasks only ---
style = task.get("style") if kind == "image" else None
if kind == "image":
    style = st.selectbox("Style", ["illustration", "photographic"], index=0 if (style or "illustration") == "illustration" else 1)
if kind == "video":
    ...
    with st.spinner("Generating (video can take a few minutes)..."):
        try:
            if kind == "image":
                ...
                if personalize_on:
                    enriched = personalize(subject, task["activity_type"], "image", style, learning_context)
                    subject = f"{task['subject']} -- {enriched}"
                result = generate_image.invoke({"subject": subject, "style": style, "out_path": out_path})
            elif kind == "audio":
                text = task["text"]
                if personalize_on:
                    text = personalize(text, task["activity_type"], "audio", "narration", learning_context)
                ...
                if personalize_on:
                    scene = personalize(scene, "video_scene", "video", "cinematic", learning_context)
